...
(function (global) {
     ...
     */
    function evaluateCraftVsBuy(auctionItem, mappedItem) {
        ...
        // 0성 노작/토드용 재료 매물(스타포스 12성 미만 & 레전드리 아님)은 직작 비교 배지 생략
        if (priceEok <= 0 || (star < 12 && potGrade !== "레전드리" && addGrade !== "레전드리")) {
            return { isEvaluated: false, priceEok };
        }
        ...
        // 1. 노작 기본가 (C_base, 억 단위)
        let baseCost = 0.5;
        if (name.includes("에테르넬")) {
            baseCost = ["모자", "상의", "하의"].includes(slot) ? 7.0 : 25.0;
        } else if (name.includes("아케인셰이드")) {
            baseCost = slot === "무기" ? 1.5 : 0.5;
        } else if (SET_ITEM_MEMBERS["칠흑의 보스 세트"].includes(name)) {
            baseCost = (name === "거대한 공포" || name === "고통의 근원") ? 25.0 : (name.includes("마도서") ? 12.0 : 18.0);
        } else if (SET_ITEM_MEMBERS["여명의 보스 세트"].includes(name)) {
            baseCost = 1.5;
        } else {
            baseCost = 0.3;
        ...
        // 2. 스타포스 기댓값 (C_star, 200제 기준 스케일링)
        let lvScale = 1.0;
        ...
        const starCost = starBase200 * lvScale;

        // 3. 잠재능력(윗잠 + 에디) 기댓값 (C_poten)
        let potCost = 0;
        if (potGrade === "레전드리") {
            potCost += reqLv >= 250 ? 18.5 : (reqLv >= 200 ? 16.5 : 14.0);
        ...
        if (topCoolSec >= 4) {
            ...
            potCost += topStatPer >= 9 ? 37.0 : 10.0;
        } else if (topCoolSec === 1) {
            potCost += topStatPer >= 18 ? 14.0 : 4.0;
        } else if (topCriDmgLines >= 2) {
            potCost += 85.0;
        } else if (topCriDmgLines === 1) {
            potCost += topStatPer >= 18 ? 28.0 : (topStatPer >= 9 ? 12.0 : 5.0);
        } else if (["무기", "보조무기", "엠블렘"].includes(slot) && topAtkBossLines >= 2) {
            potCost += topAtkBossLines >= 3 ? 42.0 : 12.0;
        } else {
            if (topStatPer >= 33) potCost += 65.0;
            else if (topStatPer >= 30) potCost += 28.0;
            else if (topStatPer >= 27) potCost += 14.0;
            else if (topStatPer >= 21) potCost += 5.5;
            else if (topStatPer >= 15) potCost += 2.0;
        }
        ...
        const rawCraftCostEok = baseCost + starCost + potCost;
        ...
        if (rawCut >= 0 && rawCut <= 20) {
            ...
            else if (rawCut >= 5) cutFactor = 0.85;
            else if (rawCut >= 3) cutFactor = 0.75;
            else cutFactor = 0.65;
        }

        const fairPriceEok = Math.max(1.0, Math.round(rawCraftCostEok * cutFactor * 10) / 10);
        const diffPct = Math.round(((priceEok - fairPriceEok) / fairPriceEok) * 100);

        let grade = "FAIR";
        let badgeText = `🟡 적정가 (추정 직작가 ${fairPriceEok}억 / ${diffPct >= 0 ? '+' : ''}${diffPct}%)`;
        let color = "#1971c2";

        if (diffPct <= -12) {
            grade = "GOOD";
            badgeText = `🟢 꿀매 (직작가 ${fairPriceEok}억 대비 ${diffPct}%)`;
            color = "#2b8a3e";
        } else if (diffPct > 12) {
            grade = "HIGH";
            badgeText = `🔴 직작 추천 (직작가 ${fairPriceEok}억 대비 +${diffPct}%)`;
            color = "#c92a2a";
        }
