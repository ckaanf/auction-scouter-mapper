...
(function (global) {
    ...
    function extractSeedRingLevel(name) {
        if (!name) return 0;
        const s = String(name).trim();
        const mLevel = s.match(/([1-4])\s*레벨/);
        if (mLevel) return parseInt(mLevel[1], 10);

        const mLv = s.match(/(?:lv|level)\.?\s*([1-4])/i);
        if (mLv) return parseInt(mLv[1], 10);

        const mEnd = s.match(/(?:링|\s)([1-4])\s*$/);
    ...
    function calculateItemFdIncrease(mappedItem, charContext) {
        ...
        if (isNewSeed) {
            ...
            if (isOldSeed && newLv > oldLv && oldLv > 0) {
                ...
                // 동일 레벨, 더 낮은 레벨, 또는 동일 계열 시드링인 경우:
                // 스탯 왜곡만 방지(변동 없음 0.0%)하고, 불필요한 동일 레벨 안내 태그는 일체 노출하지 않음
                fdPercent = 0.0;
                seedRingNotice = null;
