...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cfgPotWeight = document.getElementById('cfgPotWeight');
    ...
    function syncCraftConfigUI() {
        ...
        if (cfgPitchedBase) cfgPitchedBase.value = craftCostConfig.pitchedBaseEok ?? 18.0;
        if (cfgPotWeight) cfgPotWeight.value = craftCostConfig.potCostWeightPct ?? 100;
        ...
        document.querySelectorAll('.craft-preset-btn').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.preset === (craftCostConfig.preset || 'standard'));
        });
    }
    ...
    document.querySelectorAll('.craft-preset-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const presetKey = btn.dataset.preset || 'standard';
            const presetDef = window.FDCalculator?.CRAFT_PRESETS?.[presetKey];
            craftCostConfig.preset = presetKey;
            if (presetDef) {
                craftCostConfig.potCostWeightPct = presetDef.potCostWeightPct;
                craftCostConfig.riskMultiplier = presetDef.riskMultiplier;
            }
    ...
    [
        ...
        [cfgPitchedBase, 'pitchedBaseEok', 18.0],
        [cfgPotWeight, 'potCostWeightPct', 100],
    ...
    function renderItems() {
        ...
        preparedList.forEach(({ item, originalIndex, fdCalc, craftEval }) => {
            ...
            if (window.FDCalculator) {
                ...
                // 2) [신규 Feature 3 + 클릭 직접 지정] 직작 원가 대비 경매장 매물 손익분기(꿀매/적정/직작추천) 배지 표시
                if (craftEval && craftEval.isEvaluated) {
                    ...
                    craftDiv.style.cursor = 'pointer';
                    craftDiv.title = '클릭하여 이 매물의 기준 직작가(억)를 직접 지정하거나 자동 계산으로 복구할 수 있습니다.';
                    craftDiv.textContent = craftEval.badgeText;
                    craftDiv.onclick = (e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        const userVal = prompt(
                            `[${item.toolTip?.itemName || '장비'}]의 내 기준 직작 손익분기가(억 단위)를 입력해주세요.\n(빈칸 입력 후 확인 시 자동 기댓값 수식으로 복구됩니다)`,
