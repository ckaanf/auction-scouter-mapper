...
test('6. 직작 vs 완제 손익분기 가치평가(evaluateCraftVsBuy): 에테르넬 쿨뚝/아케인 크뎀장갑 및 가횟 감가 검증', () => {
    ...
    // 6-4. 사용자 대시보드 커스텀 설정(customConfig) 및 개별 매물 직접 지정(customItemOverrides) 검증
    const eventConfigEval = evaluateCraftVsBuy(
        { tradeSn: '9999', price: 7500000000 },
        eternalCoolHat,
        { preset: 'full_event', eternalBaseEok: 4.0, potCostWeightPct: 60 }
    );
    // 노작 4억 + 풀 이벤트 할인 + 잠재 60% 반영 시 기준 직작가가 크게 낮아져 75억이 HIGH(직작 추천)로 바뀜
    assert.equal(eventConfigEval.grade, 'HIGH');
    ...
    // 유저가 직접 기준 직작가를 95억으로 지정하면 75억은 즉시 꿀매(GOOD) + [직접지정] 표시
    ...
    assert.ok(pinpointOverrideEval.badgeText.includes('[직접지정]'));
});
